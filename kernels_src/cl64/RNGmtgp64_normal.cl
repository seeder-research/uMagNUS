    ...
    int size)
{
    ...
    double normf_[6];
    // main loop
    for (int i = 0; i < size / 2; i += MTGP64_LS) {
	r = para_rec64(&mtgp,
	...
	barrier(CLK_LOCAL_MEM_FENCE);
        ...
        unif[0] = ulong2double(tmpNum[0], tmpNum[1]);
        unif[1] = ulong2double(tmpNum[2], tmpNum[3]);
        unif[2] = ulong2double(tmpNum[4], tmpNum[5]);
        unif[3] = ulong2double(tmpNum[6], tmpNum[7]);
        unif[4] = ulong2double(tmpNum[8], tmpNum[9]);
        unif[5] = ulong2double(tmpNum[10], tmpNum[11]);

	boxMuller64(unif, normf_, 0);
	boxMuller64(unif, normf_, 1);
	boxMuller64(unif, normf_, 2);
