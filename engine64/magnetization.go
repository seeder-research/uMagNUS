...
func (m *magnetization) normalize()              { opencl.Normalize(m.Buffer(), geometry.Gpu()) }
...
func (m *magnetization) SetCell(ix, iy, iz int, v data.Vector) {
	for c := 0; c < 3; c++ {
		opencl.SetCell(m.Buffer(), c, ix, iy, iz, float64(v[c]))
	}
}
		...
		for iy := 0; iy < n[Y]; iy++ {
			for ix := 0; ix < n[X]; ix++ {
				...
				if region(x, y, z) { // inside
					m := conf(x, y, z)
					h[X][iz][iy][ix] = float64(m[X])
					h[Y][iz][iy][ix] = float64(m[Y])
					h[Z][iz][iy][ix] = float64(m[Z])
				}
			}
		}
	}
