...
func (geometry *geom) setGeom(s Shape) {
	...
	geometry.shape = s
	if geometry.Gpu().IsNil() {
		geometry.buffer = opencl.NewSlice(1, geometry.Mesh().Size())
	}
...
func (g *geom) cellVolume(ix, iy, iz int) float64 {
	r := Index2Coord(ix, iy, iz)
	x0, y0, z0 := r[X], r[Y], r[Z]
...
func (g *geom) shift(dx int) {
	...
	newv := float64(1) // initially fill edges with 1's
