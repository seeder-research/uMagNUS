...
func k_{{.Name}}_async ( {{range $i, $t := .ArgT}}{{index $.ArgN $i}} {{$t}}, {{end}} cfg *config, events []*cl.Event, launchQueue *cl.CommandQueue) *cl.Event {
...
//	args := {{.Name}}_args.argptr[:]
	event := LaunchKernel("{{.Name}}", cfg.Grid, cfg.Block, events, launchQueue)
