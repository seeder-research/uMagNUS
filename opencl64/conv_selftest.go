...
// 	(O1) = (K11 K12 K13) * (I1)
// 	(O2)   (K21 K22 K23)   (I2)
func bruteConv(in, out [3][][][]float64, kernel [3][3]*data.Slice) {

	var kern [3][3][][][]float64
	for i := range kern {
		for j := range kern[i] {
			if kernel[i][j] != nil {
...
func initConvTestInput(input [3][][][]float64) {
...
func sizeOf(block [][][]float64) [3]int {
