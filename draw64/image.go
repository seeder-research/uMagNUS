...
func parseMinMax(f *data.Slice, fmin, fmax string) (min, max float64) {
...
func drawVectors(img *image.RGBA, arr [3][][][]float64, arrowSize int) {
...
func extrema(data []float64) (min, max float64) {
	min = data[0]
	max = data[0]
	for _, d := range data {
		if d < min {
...
// averages data over X (usually thickness of thin film)
func drawFloats(img *image.RGBA, arr [][][]float64, min, max float64, colormap ...color.RGBA) {
