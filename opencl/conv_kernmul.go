...
func kernMulRSymm3D_async(fftM [3]*data.Slice, Kxx, Kyy, Kzz, Kyz, Kxz, Kxy *data.Slice, Nx, Ny, Nz int, queue *cl.CommandQueue, events []*cl.Event) {
	...
	k_kernmulRSymm3D_async(fftM[X].DevPtr(0), fftM[Y].DevPtr(0), fftM[Z].DevPtr(0),
		...
		Nx, Ny, Nz, cfg, queue, events)
}

// kernel multiplication for 2D demag convolution on X and Y, exploiting full kernel symmetry.
func kernMulRSymm2Dxy_async(fftMx, fftMy, Kxx, Kyy, Kxy *data.Slice, Nx, Ny int, queue *cl.CommandQueue, events []*cl.Event) {
	...
	k_kernmulRSymm2Dxy_async(fftMx.DevPtr(0), fftMy.DevPtr(0),
		...
		Nx, Ny, cfg, queue, events)
}

// kernel multiplication for 2D demag convolution on Z, exploiting full kernel symmetry.
func kernMulRSymm2Dz_async(fftMz, Kzz *data.Slice, Nx, Ny int, queue *cl.CommandQueue, events []*cl.Event) {
...
// Used for MFM images.
func kernMulC_async(fftM, K *data.Slice, Nx, Ny int, queue *cl.CommandQueue, events []*cl.Event) {
