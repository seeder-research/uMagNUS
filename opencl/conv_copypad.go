...
func copyUnPad(dst, src *data.Slice, dstsize, srcsize [3]int, queue *cl.CommandQueue, events []*cl.Event) {
...
func copyPadMul(dst, src, vol *data.Slice, dstsize, srcsize [3]int, Msat MSlice, queue *cl.CommandQueue, events []*cl.Event) {
	util.Argument(dst.NComp() == 1 && src.NComp() == 1)
