...
func copyUnPad(dst, src *data.Slice, dstsize, srcsize [3]int, q *cl.CommandQueue, ewl []*cl.Event) {
	...
	event := k_copyunpad_async(dst.DevPtr(0), dstsize[X], dstsize[Y], dstsize[Z],
		src.DevPtr(0), srcsize[X], srcsize[Y], srcsize[Z], cfg, ewl,
		q)
...
func copyPadMul(dst, src, vol *data.Slice, dstsize, srcsize [3]int, Msat MSlice, q *cl.CommandQueue, ewl []*cl.Event) {
	// synchronization should be done by code using
	// opencl library
	...
	event := k_copypadmul2_async(dst.DevPtr(0), dstsize[X], dstsize[Y], dstsize[Z],
		...
		Msat.DevPtr(0), Msat.Mul(0), vol.DevPtr(0), cfg, ewl,
		q)
