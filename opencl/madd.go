...
func Mul(dst, a, b *data.Slice, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
	...
	for c := 0; c < nComp; c++ {
		k_mul_async(dst.DevPtr(c), a.DevPtr(c), b.DevPtr(c), N, cfg, queue[c], events)
	}
}

// divide: dst[i] = a[i] / b[i]
// divide-by-zero yields zero.
func Div(dst, a, b *data.Slice, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
	util.Assert(a.Len() == N && a.NComp() == nComp && b.Len() == N && b.NComp() == nComp && len(queue) == nComp)
	cfg := make1DConf(N)
	...
	for c := 0; c < nComp; c++ {
		k_pointwise_div_async(dst.DevPtr(c), a.DevPtr(c), b.DevPtr(c), N, cfg, queue[c], events)
	}
}

// Add: dst = src1 + src2.
func Add(dst, src1, src2 *data.Slice, queue []*cl.CommandQueue, events []*cl.Event) {
	Madd2(dst, src1, src2, 1, 1, queue, events)
}

// multiply-add: dst[i] = src1[i] * factor1 + src2[i] * factor2
func Madd2(dst, src1, src2 *data.Slice, factor1, factor2 float32, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
...
func Madd3(dst, src1, src2, src3 *data.Slice, factor1, factor2, factor3 float32, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
...
func Madd4(dst, src1, src2, src3, src4 *data.Slice, factor1, factor2, factor3, factor4 float32, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
...
func Madd5(dst, src1, src2, src3, src4, src5 *data.Slice, factor1, factor2, factor3, factor4, factor5 float32, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
...
func Madd6(dst, src1, src2, src3, src4, src5, src6 *data.Slice, factor1, factor2, factor3, factor4, factor5, factor6 float32, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
...
func Madd7(dst, src1, src2, src3, src4, src5, src6, src7 *data.Slice, factor1, factor2, factor3, factor4, factor5, factor6, factor7 float32, queue []*cl.CommandQueue, events []*cl.Event) {
	N := dst.Len()
	nComp := dst.NComp()
