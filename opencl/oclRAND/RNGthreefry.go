...
func (p *THREEFRY_status_array_ptr) Init(seed uint64, events []*cl.Event, queue *cl.CommandQueue) {
	...
	event := k_threefry_seed_async(unsafe.Pointer(p.Status_key), unsafe.Pointer(p.Status_counter),
		...
		&config{[]int{totalCount}, []int{p.GetGroupSize()}}, []*cl.Event{seed_event}, queue)
...
func (p *THREEFRY_status_array_ptr) GenerateUniform(d_data unsafe.Pointer, data_size int, events []*cl.Event, queue *cl.CommandQueue) *cl.Event {
...
func (p *THREEFRY_status_array_ptr) GenerateNormal(d_data unsafe.Pointer, data_size int, events []*cl.Event, queue *cl.CommandQueue) *cl.Event {
